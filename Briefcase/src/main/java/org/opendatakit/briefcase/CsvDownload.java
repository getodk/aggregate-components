...
public class CsvDownload {
	...
	public interface ActionListener {
		 ...
		 * @param count number of cursor fetches for this formId.
		 */
	...
	/** final path element for accessing csv Fragments on ODK Aggregate 1.0 */
	private static final String CSV_FRAGMENT_URL_PATH = "view/csvFragment";
	/** final path element for accessing binary data on ODK Aggregate 1.0 */
	private static final String BINARY_DATA_URL_PATH = "view/binaryData";
	...
	static UrlType classifyUrl(String unknownUrl) {
		try {
			...
			if (idx != -1) {
				// In Aggregate 1.0, the path is one deep, so 
				// find the idx before this...
				idx = path.lastIndexOf('/', idx-1);
				if ( idx == -1 ) return UrlType.UNKNOWN_URL;
	...
	static String extractKeyReference(String keyUrl)
		...
		for (String kvPair : query.split(PARAM_DELIMITER)) {
			...
			if (key.equals(FORM_ID)) {
				if (formId != null) {
					throw new IllegalArgumentException(FORM_ID
							+ " occurs more than once in supplied url: "
							+ keyUrl);
				}
				formId = value;
	...
	/**
	 * Called after fetch is complete. Writes the formId and fileName mappings to
	...
	public void fetchCsvRecursively(String urlArg, BinaryDataTreatment fetchBinaryData,
		...
		//
		// if we are pulling a form with this formId structure of repeat groups:
		...
		// If the formId does not include a slash, perform a fetch of a
		// single record to determine the actual formId. This is needed
		// to maintain the directory structure in our local tree.
		String formId = extractKeyReference(urlArg);
